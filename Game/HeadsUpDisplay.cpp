...
namespace GameDev2D
{
    ...
    float HeadsUpDisplay::LoadContent()
    {
        ...
        float startingY = m_World->GetTileSize() * m_World->GetNumberOfVerticalTiles();
        ...
        Sprite* bombSprite = new Sprite("MainAtlas", "Bomb");
        bombSprite->SetAnchorPoint(1.0f, 0.0f);
        bombSprite->SetLocalPosition(screenWidth * HUD_BOMB_SPRITE_POSITION_PCT.x, screenHeight * HUD_BOMB_SPRITE_POSITION_PCT.y);
        bombSprite->SetLocalScale(0.5f, 0.5f);
        AddGameObject(bombSprite);

        //Create the rupee label
        m_BombLabel = new Label("slkscr", "ttf", 32);
        m_BombLabel->SetAnchorPoint(0.0f, 0.0f);
        m_BombLabel->SetLocalPosition(bombSprite->GetLocalPosition() + vec2(8.0f, 0.0f));
        AddGameObject(m_BombLabel);
    ...
    void HeadsUpDisplay::Update(double aDelta)
    {
        if(m_World != nullptr && m_World->GetHero() != nullptr)
        {
            ...
            //Update the bomb label
            stringstream ss2;
            ss2 << m_World->GetHero()->GetBombs();
            m_BombLabel->SetText(ss2.str());
        }
